...
option_list = list(
  ...
  make_option(c("-g", "--gtf_file"), type = "character", default = NULL,
              ...
              metavar = "character"),  
  make_option(c("-c", "--count_table"), type = "character", default = "data/reference_data/seurat_object_anne_raw.rds",
              help = "Seurat object with raw counts from the sample of interest",
              metavar = "character"),
...
required_args <- c("input_csv", "subsetted_bam", "count_table","out_directory", 
...
setwd('/Users/4472241/scCode/make_primers/')
# read table with variants
raw_variants <- read_csv(opt$input_csv)
#raw_variants <- read_csv('./CMML_9/CMML_9_variants.csv')
...
#gtf_genes <- gffReadGR('./data/Homo_sapiens.GRCh38.100.chr.gtf')
...
count_matrix <- readRDS(opt$count_table)
#count_matrix <- readRDS('./CMML_9/CMML9_SF-100109-106293_2020-10-15.rds')
...
if (is.null(dim(count_matrix@assays$RNA@counts[gene_names,]))){
  counts <- c(sum(count_matrix@assays$RNA@counts[gene_names,]))
  gene_expression <- data.frame(symbol = gene_names,
...
#Exclude because it makes Chromosome into "chr##" when we need "##"
...
register(MulticoreParam(workers = 1)) #opt$cores))


# infer polyA sites. With the subsetted BAM it takes a few seconds
polyA_sites <- inferPolyASites(target_exons,
                               bam = opt$subsetted_bam, #'./CMML_9/CMML_9.subset.bam', 
                               polyA_downstream = 50, by = 1,
                               wdsize = 100, min_cvrg = 100, parallel = FALSE)
...
raw_variants$CHROM <- as.character(raw_variants$CHROM)
...
if(F){
  ...
  annotated_variants <- annotated_variants %>% left_join(flag_table) %>% 
    ...
    select(symbol, CHROM, POS, REF, ALT, consequence,
           counts_cell, distance_3_end, af_tcells, af_tumor, flag,
           cancer_gene, cosmic_id, FILTER, read_depth) %>% 
...
write_csv(annotated_variants, file = paste0(opt$out_directory, opt$outfile))
#outFile <- 'annotated_variants.csv'
#write_csv(annotated_variants, file = paste0(outDir, outFile))