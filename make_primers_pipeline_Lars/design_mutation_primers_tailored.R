...
get_GoT_regions <- function(genomic_sites, contig_list, gene_names, gtf_file, polyAs){
  ...
  #Note: we remove this because our style is not "chr##" but just "##"
...
#NOW convert to UCSC style for seqnames (chr##)
# make seqlevels homogeneous
seqlevelsStyle(ranges_list) <- "UCSC"
...
outer_primers <- TAPseqInput(sequences,
                             product_size_range = c((81+250), (81+350)), 
                             primer_num_return = 5,
...
middle_primers <- TAPseqInput(sequences,
                              product_size_range = c((81+100), (81+200)), 
                              primer_num_return = 5,
                              target_annot = ranges_list)
...
inner_primers <- TAPseqInput(sequences,
                             product_size_range = c(81, 81+opt$read_length-10), 
                             primer_num_return = 5,
                             target_annot = ranges_list)
...
write_csv(hits_table %>% dplyr::rename(ref = Reference_Allele, alt = Tumor_Seq_Allele2), 
