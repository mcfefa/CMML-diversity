...
colorMapToSample <- function(vec, min=0, max=38){
  output <- c()
  for (i in 1: length(vec)){
    r <- 0
    b <- (vec[i])/(max)
...
laml@clinical.data <- clin_data_sorted
...
pdf(file = "/Users/4472241/scCode/oncoPlots/oncoPlot_discrete_WithTimeToSample_onlyTimeOfSample_orderByEppertHSC.pdf", width = 12, height = 10, paper = "special", bg = "white")
...
#Oncoplot with clinical data
pdf(file = "/Users/4472241/scCode/oncoPlots/oncoPlot_continuous_WithTimeToSample_onlyTimeOfSample_orderByEppertHSC.pdf", width = 12, height = 10, paper = "special", bg = "white")
oncoplot(maf = laml, clinicalFeatures = c("Blast_Percent", "Eppert_HSC", "Time_From_Diagnosis_to_Sample"),
         sampleOrder = clin_data_sorted$Tumor_Sample_Barcode, top = 20,
         annotationColor = colsAnn,
         showTumorSampleBarcodes = TRUE, removeNonMutated = FALSE,
         anno_height = 9/4, barcode_mar = 8)
dev.off()
