...
#Set dir
dir <- '/share/lab_altrock/MeghanCluster/BrianCluster/BAM/mitocloneOutput_fourthTen_defaultParams_MINREADS3_MINCELL10_MINFRAC0.05_MINRelative.Other0.99/' #For cluster
setwd(dir)
bamdir <- '/share/lab_altrock/MeghanCluster/BrianCluster/BAM/fourthTenSamples_RustOutput' #For cluster
...
#saveRDS(countTables, paste0(dir, '/countTablesFourthTen.rds'))
countTables <- readRDS(paste0(dir, '/countTablesFourthTen.rds'))
...
#Check first 3
print(colnames(result$CMML_31@M))
print(colnames(result$CMML_32@M))
print(colnames(result$CMML_33@M))
print(colnames(result$CMML_34@M))
print(colnames(result$CMML_35@M))
print(colnames(result$CMML_36@M))
print(colnames(result$CMML_37@M))
print(colnames(result$CMML_38@M))
print(colnames(result$CMML_39@M))
print(colnames(result$zheng@M))

#Assign patient 31 through 39 + zheng
P31 <- result$CMML_31
P32 <- result$CMML_32
P33 <- result$CMML_33
P33@cluster[which((gsub(".*\\.\\.", "", colnames(result$CMML_33@M)) %in% ""))] <- F
P34 <- result$CMML_34
P34@cluster[which((gsub(".*\\.\\.", "", colnames(result$CMML_34@M)) %in% ""))] <- F
P35 <- result$CMML_35
P36 <- result$CMML_36
P37 <- result$CMML_37
P37@cluster[which((gsub(".*\\.\\.", "", colnames(result$CMML_37@M)) %in% ""))] <- F
P38 <- result$CMML_38
P39 <- result$CMML_39
zheng <- result$zheng
...
P31 <- muta_cluster(P31, cores = 12, tempfolder = paste0(getwd(),"/CMML_31_temp"), force_recalc = T)
saveRDS(P31, paste0(dir, 'P31_results_mitoclone.rds'))
P32 <- muta_cluster(P32, cores = 12, tempfolder = paste0(getwd(),"/CMML_32_temp"), force_recalc = T)
saveRDS(P32, paste0(dir, 'P32_results_mitoclone.rds'))
P33 <- muta_cluster(P33, cores = 12, tempfolder = paste0(getwd(),"/CMML_33_temp"), force_recalc = T)
saveRDS(P33, paste0(dir, 'P33_results_mitoclone.rds'))
P34 <- muta_cluster(P34, cores = 12, tempfolder = paste0(getwd(),"/CMML_34_temp"), force_recalc = T)
saveRDS(P34, paste0(dir, 'P34_results_mitoclone.rds'))
P35 <- muta_cluster(P35, cores = 12, tempfolder = paste0(getwd(),"/CMML_35_temp"), force_recalc = T)
saveRDS(P35, paste0(dir, 'P35_results_mitoclone.rds'))
P36 <- muta_cluster(P36, cores = 12, tempfolder = paste0(getwd(),"/CMML_36_temp"), force_recalc = T)
saveRDS(P36, paste0(dir, 'P36_results_mitoclone.rds'))
P37 <- muta_cluster(P37, cores = 12, tempfolder = paste0(getwd(),"/CMML_37_temp"), force_recalc = T)
saveRDS(P37, paste0(dir, 'P37_results_mitoclone.rds'))
P38 <- muta_cluster(P38, cores = 12, tempfolder = paste0(getwd(),"/CMML_38_temp"), force_recalc = T)
saveRDS(P38, paste0(dir, 'P38_results_mitoclone.rds'))
P39 <- muta_cluster(P39, cores = 12, tempfolder = paste0(getwd(),"/CMML_39_temp"), force_recalc = T)
saveRDS(P39, paste0(dir, 'P39_results_mitoclone.rds'))
zheng <- muta_cluster(zheng, cores = 12, tempfolder = paste0(getwd(),"/zheng_temp"), force_recalc = T)
saveRDS(zheng, paste0(dir, 'zheng_results_mitoclone.rds'))